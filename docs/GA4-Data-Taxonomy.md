...
| `banner_open` | 미구현 | 배너 클릭 효율 측정 | `banner_id`, `banner_title`, `banner_position` | |
...
| `template_create_start` | 미구현 | 새 템플릿 생성 진입 | `template_origin`=`default\|empty` | P1 |
| `template_name_edit` | 미구현 | 에디터 사용성 파악 | `template_id` | P3 |
| `template_item_add` | 구현됨 | 에디터 내 핵심 편집 행위 | `add_method`(`drag`\|`button`), `template_id?` | P1 |
| `template_item_update` | 미구현 | 링크/아이콘/속성 수정 | `update_type`, `template_id` | P2 |
| `template_item_delete` | 미구현 | 아이템 삭제 행위 | `delete_source`, `template_id` | P2 |
...
| `template_delete` | 미구현 | 템플릿 삭제 | `template_id`, `template_origin`, `sync_status` | P2 |
...
| `alerts_subscription_change` | 미구현 | 개인화 기능 사용 | `category`, `result` | P3 |
...
| `labs_view_open` | 미구현 | Labs 진입 | `feature_name` | P3 |
| `labs_feature_use` | 미구현 | Labs 세부 기능 사용 | `feature_name`, `result` | P3 |

## Settings / System Events

| Event Name | 상태 | 목적 | 주요 Params | 우선순위 |
| --- | --- | --- | --- | --- |
| `settings_open` | 미구현 | 설정 진입 측정 | `entry_point` | P2 |
