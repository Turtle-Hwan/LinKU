...
닉네임 생성 등 제품 규칙은 TypeScript에서 관리합니다. SQL은 저장·조회, 접근 권한,
데이터 무결성과 원자적 동시성 처리를 담당하며 제품 표현이나 생성 규칙을 포함하지 않습니다.
브라우저에서 우회할 수 있는 입력 형식과 계정별 저장 한도는 DB에서도 검증합니다.
...
- 빈 템플릿도 개인 저장·동기화할 수 있지만, 공개 게시·업데이트에는 캔버스의 링크가
  하나 이상 필요합니다. 임시 저장 공간의 링크는 세지 않으며, 빈 편집본의 게시를
  거부해도 기존 공개 snapshot은 유지됩니다.
...
작성자 닉네임은 `profiles`에서 조인하므로 이름을 바꿔도 게시 snapshot이나 revision을
수정하지 않습니다. 비공개 아이콘 경로는 소유자 UUID와 content hash로 계산하며,
파일 크기·형식 제한은 Storage bucket에서 적용합니다.
