...
`src/assets/banners/banner.json`의 각 배너에는 ISO 8601 형식의 `startAt`과
`endAt`을 선택적으로 지정할 수 있습니다.
...
- `startAt`은 배너 노출 시작 시각을 포함합니다.
- `endAt`은 배너 노출 종료 시각을 포함하지 않습니다.
- 생략한 시작 또는 종료 시각은 해당 방향으로 제한이 없음을 뜻합니다.
- 사용자 환경에 관계없이 같은 시각에 전환되도록 `Z` 또는 `+09:00` 같은
  timezone을 반드시 포함하세요.
- 날짜 형식이 잘못된 배너는 의도치 않은 노출을 막기 위해 표시하지 않습니다.
- 날짜 변경은 `main` 반영 후 GitHub Pages 배포가 완료되어야 적용됩니다.
- 기간 필터를 지원하지 않는 이전 확장 프로그램에서도 즉시 내려야 하는 배너는
  기간 종료만 설정하지 말고 `banner.json`에서 항목을 제거하세요.
