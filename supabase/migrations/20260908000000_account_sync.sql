...
create function public.initialize_profile(p_nickname text)
    ...
    where item -> 'icon' ->> 'kind' = 'asset'
      and not exists (
        ...
        where bucket_id = 'published-template-assets'
          and name = p_template_id::text || '/' || (item -> 'icon' ->> 'hash') || '.webp'
      )
  ) then
    raise exception using errcode = '40001', message = 'LINKU_CONFLICT';
  end if;
...
create function linku_private.can_delete_published_asset(target_name text)
returns boolean
language plpgsql
volatile
security definer
...
begin
  ...
  return exists (
    select 1 from public.templates
    where id::text = (storage.foldername(target_name))[1]
      and owner_id = current_user_id
  ) and not exists (
    select 1 from public.template_publications publication,
      jsonb_array_elements(publication.snapshot -> 'items') item
    where publication.template_id::text = (storage.foldername(target_name))[1]
      and publication.unpublished_at is null
      and item -> 'icon' ->> 'kind' = 'asset'
      and target_name = publication.template_id::text || '/' || (item -> 'icon' ->> 'hash') || '.webp'
  );
end;
$$;

revoke all on function linku_private.can_delete_published_asset(text) from public;
grant execute on function linku_private.can_delete_published_asset(text) to authenticated;
...
using (
  ...
  and storage.filename(name) ~ '^[0-9a-f]{64}[.]webp$'
  and linku_private.is_google_session()
  and linku_private.can_delete_published_asset(name)
);
