...
-- Exercise Storage API deletion policies without changing its production guard.
...
select public.initialize_profile('테스트 사용자');
...
where bucket_id = 'published-template-assets'
  and name = 'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb/' || repeat('e', 64) || '.webp';
select throws_ok(
  $$select public.publish_template('bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb', repeat('b', 64))$$,
  '40001', 'LINKU_CONFLICT', 'publication requires uploaded snapshot icons'
);
insert into storage.objects (bucket_id, name, owner_id) values (
  'published-template-assets',
  'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb/' || repeat('e', 64) || '.webp',
...
select lives_ok(
  $$select public.publish_template('bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb', repeat('b', 64))$$,
  'publication succeeds with its exact content and uploaded icons'
);
delete from storage.objects
where bucket_id = 'published-template-assets'
  and name = 'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb/' || repeat('e', 64) || '.webp';
select is(
  (select count(*)::integer from storage.objects
    where bucket_id = 'published-template-assets'
      and name = 'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb/' || repeat('e', 64) || '.webp'),
  1, 'stale device cleanup cannot delete an active publication icon'
);
select public.unpublish_template('bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb', 1);
delete from storage.objects
where bucket_id = 'published-template-assets'
  and name = 'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb/' || repeat('e', 64) || '.webp';
select is(
  (select count(*)::integer from storage.objects
    where bucket_id = 'published-template-assets'
      and name = 'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb/' || repeat('e', 64) || '.webp'),
  0, 'unpublished icons can still be cleaned up'
);
...
select throws_ok(
  $$select public.publish_template('aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', repeat('f', 64))$$,
  '40001', 'LINKU_CONFLICT', 'publishing rejects a different device draft'
);
select lives_ok(
  $$select public.publish_template('aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', repeat('a', 64))$$,
...
select throws_ok(
  $$select public.publish_template('aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', repeat('a', 64), 1)$$,
  '40001', 'LINKU_CONFLICT', 'stale source hash cannot update a publication'
);
select lives_ok(
  $$select public.publish_template('aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', repeat('c', 64), 1)$$,
