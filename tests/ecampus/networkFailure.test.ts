...
test("오프라인과 fetch 차단을 예상 가능한 전송 실패로 분류한다", () => {
  ...
  assert.equal(isExpectedNetworkFailure(new TypeError("Load failed"), true), true);
});

test("파싱과 도메인 오류는 네트워크 실패로 오인하지 않는다", () => {
  assert.equal(classifyNetworkFailure(new SyntaxError("Unexpected token"), true), "unknown");
  assert.equal(classifyNetworkFailure(new SyntaxError("Unexpected token"), false), "unknown");
  assert.equal(classifyNetworkFailure(new Error("Load failed"), false), "unknown");
...
test("중단된 요청을 별도 전송 상태로 분류한다", () => {
  ...
  assert.equal(classifyNetworkFailure(error, false), "aborted");
});
